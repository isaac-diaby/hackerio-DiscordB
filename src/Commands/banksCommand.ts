...
export class BanksCommand extends DiscordCommand {
  ...
  listAvailableBanks(userData: IUserState) {
    ...
    Msg.setTitle(
      "Hacking Banks Are Very Rewarding, However, Also Comes With A Risk and a Cost."
    )
      .addField(
        "Bank Names",
        [...BanksCommand.BANKS_META.map(banks => banks.name)],
        true
      )
      .addField(
        "Required Level",
        [...BanksCommand.BANKS_META.map(banks => banks.minPlayerLevel)],
        true
      )
      .addField(
        "Required Money",

...
export class EliteCommand extends DiscordCommand {
                ...
                switch (this.args[0]) {
                    ...
                    default:
                        const Msg = new Discord.RichEmbed()
                            ...
                            .addField('EP perks ⭐', ['- Enemies have a  (level*0.072) =  just over 13%+ disadvantage success rate when trying to hack you!',
                            ...
                            .setFooter(`To join the elte list type ${process.env.BOT_PREFIX}elite -j`)
    ...
    joinElite(userID: string) {
        ...
        const Msg = new Discord.RichEmbed().setColor('#60BE82').setTitle('How To Become Elite').setDescription('follow these simple steps: ')
            .addField('1. Join The Official Sever', 'http://bit.ly/CGBofficialServer')
            .addField('2. Donate To Get The Title: HackerIO Elite ', 'http://bit.ly/CGBdonate')
            .setFooter('Then type this command again to active Elite')
        const isUserInOfficialServer = this.botClient.guilds.get('566982444822036500').members.get(this.msg.author.id)
        if (isUserInOfficialServer !== undefined) {
            ...
            if (!isUserInOfficialServer.roles.has('605180133535645745')) return this.msg.author.send(Msg)
    ...
    static altEliteStatus(userID: string, isElite: boolean, user: Discord.User) {
        const Msg = new Discord.RichEmbed().setColor('#F44336').setTitle('Elite Update');
        (isElite) ?  Msg.setDescription('Welcome to the elite ⚔.').setColor('#60BE82') : Msg.setDescription('Expired Membership to elite ⌛').addField('Rejoin', 'http://bit.ly/CGBdonate' );
                    UserMD.findOneAndUpdate({userID}, {
